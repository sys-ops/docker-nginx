...
if [ ${#} -eq 0 ]; then
  ...
  if [ "${MAINTENANCE}" == "true" ]; then
    ...
    perl -p -i -e "s/back later/back later<p>${HOSTNAME} container and ${HOST} host and ${HOST_SYSTEM} system<BR \/>Message of the day:<BR \/>${MOTD}<BR \/>lastlog:<font size="-10"><BR \/>\'${LASTLOG}\'<\/font>/" /var/www/html/index.html
  fi
